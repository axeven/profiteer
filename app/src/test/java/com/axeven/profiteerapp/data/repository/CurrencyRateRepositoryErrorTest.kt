...
/**
 * TDD Tests for CurrencyRateRepository error handling refactoring.
 *
 * **Phase**: RED (Failing Tests)
 * **Purpose**: Verify that CurrencyRateRepository no longer calls SharedErrorViewModel
 ...
 * They will fail until CurrencyRateRepository is refactored in Phase 3.
 *
 * **Error Coverage**:
 * - Error #1: getUserCurrencyRates() - Firestore listener error (1 occurrence)
 *
 ...
 */
class CurrencyRateRepositoryErrorTest {

    // region Error #1: getUserCurrencyRates() Listener Error

    /**
     * Test: getUserCurrencyRates() should return RepositoryError on listener failure.
     *
     * **Error Context**: Error #1 from docs/repository-error-mapping.md
     * - Location: CurrencyRateRepository.kt:43-48
     * - Trigger: Firestore snapshot listener fails
     * - User Impact: Currency rates not loading in UI
     ...
     */
    @Test
    fun `getUserCurrencyRates should return RepositoryError on listener failure`() {
        fail(
            ...
            "\n" +
            "To implement, see: CurrencyRateRepository.kt:43-48"
        )
    }

    /**
     * Test: getUserCurrencyRates() should return NetworkError when offline.
     */
    @Test
    fun `getUserCurrencyRates should return NetworkError when offline`() {
        fail(
            "TEST NOT YET IMPLEMENTED: Requires repository refactoring in Phase 3.\n" +
            "Expected behavior:\n" +
            ...
            "\n" +
            "See: CurrencyRateRepository.kt:43-48"
        )
    }

    /**
     * Test: getUserCurrencyRates() should return AuthenticationError when auth fails.
     */
    @Test
    fun `getUserCurrencyRates should return AuthenticationError when auth fails`() {
        fail(
            "TEST NOT YET IMPLEMENTED: Requires repository refactoring in Phase 3.\n" +
            "Expected behavior:\n" +
            ...
            "\n" +
            "See: CurrencyRateRepository.kt:39-48"
        )
    }

    /**
     * Test: getUserCurrencyRates() error should preserve all context.
     */
    @Test
    fun `getUserCurrencyRates error should preserve FirestoreErrorHandler context`() {
        fail(
            "TEST NOT YET IMPLEMENTED: Requires repository refactoring in Phase 3.\n" +
            "Expected behavior:\n" +
    ...
    /**
     * Test: CurrencyRateRepository should NOT have SharedErrorViewModel dependency.
     *
     * **Current Status**: 🔴 FAIL (SharedErrorViewModel is in constructor)
     * **After Refactoring**: 🟢 PASS (SharedErrorViewModel removed)
     */
    @Test
    fun `CurrencyRateRepository should not depend on SharedErrorViewModel`() {
        val constructorParams = CurrencyRateRepository::class.java.constructors[0]
            .parameterTypes
            .toList()
        ...
        assertFalse(
            "CurrencyRateRepository should not have SharedErrorViewModel dependency.\n" +
            ...
            "\n" +
            "See: CurrencyRateRepository.kt:22-27",
     ...
     */
    @Test
    fun `CurrencyRateRepository should only have data layer dependencies`() {
        val constructorParams = CurrencyRateRepository::class.java.constructors[0]
            .parameterTypes
            .map { it.simpleName }
        ...
        assertTrue(
            "CurrencyRateRepository has UI layer dependencies: $foundUIDependencies\n" +
    ...
    fun `document expected ViewModel integration pattern`() {
        val expectedPattern = """
            // In ViewModel:
            repository.getUserCurrencyRates(userId)
                .catch { error ->
                    when (error) {
